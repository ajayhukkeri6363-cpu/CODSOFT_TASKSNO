...
export async function ensureTablesExist() {
  ...
  for (const ddl of ddlStatements) {
    try {
      ...
      // Ignore if table already exists or DDL is handled differently
    }
  }
}

export async function seedCareerHubData() {
  ...
  const techcorp = await prisma.company.upsert({
    where: { slug: 'techcorp-innovations' },
    update: {},
    create: {
  ...
  const innovatelabs = await prisma.company.upsert({
    where: { slug: 'innovatelabs-ai' },
    update: {},
    create: {
  ...
  const cloudscale = await prisma.company.upsert({
    where: { slug: 'cloudscale-systems' },
    update: {},
    create: {
  ...
  const pixelcraft = await prisma.company.upsert({
    where: { slug: 'pixelcraft-studio' },
    update: {},
    create: {
  ...
  const recruiter1 = await prisma.user.upsert({
    where: { email: 'sarah.connor@techcorp.com' },
    update: {},
    create: {
  ...
  const recruiter2 = await prisma.user.upsert({
    where: { email: 'david.kim@innovatelabs.io' },
    update: {},
    create: {
  ...
  // 5. Create 16 Diverse Job Listings
  const jobsData = [
    {
    ...
    },
    {
      ...
      salaryCurrency: 'USD',
      isFeatured: false,
      status: 'DRAFT', // Draft job for recruiter management
  ...
  // 6. Create Applications across stages
  const alexUser = candidateUsers[0]; // Alex Morgan
  const priyaUser = candidateUsers[1]; // Priya Sharma
  const jamesUser = candidateUsers[2]; // James Wilson
  const elenaUser = candidateUsers[3]; // Elena Rostova
  const michaelUser = candidateUsers[4]; // Michael Chang
  ...
  // Alex applies to Senior Full Stack at TechCorp -> INTERVIEW stage with scheduled interview!
  const appAlex1 = await prisma.application.upsert({
    where: {
      jobId_candidateId: {
        jobId: createdJobs[0].id, // Senior Full Stack
  ...
  // Priya applies to Lead Frontend Architect at TechCorp -> SHORTLISTED
  ...
  // Elena applies to AI Infrastructure at InnovateLabs -> UNDER_REVIEW
  await prisma.application.upsert({
    where: {
      jobId_candidateId: {
  ...
  // James applies to Cloud Platform SRE at CloudScale -> SELECTED
  await prisma.application.upsert({
    where: {
      jobId_candidateId: {
  ...
  // Michael applies to Senior Product Designer at PixelCraft -> APPLIED
  await prisma.application.upsert({
    where: {
      jobId_candidateId: {
  ...
  await prisma.savedJob.upsert({
    where: {
      candidateId_jobId: {
        candidateId: alexUser.id,
        jobId: createdJobs[2].id, // AI Infrastructure
      },
    },
    update: {},
    create: {
  ...
  await prisma.savedJob.upsert({
    where: {
      candidateId_jobId: {
        candidateId: alexUser.id,
        jobId: createdJobs[5].id, // Distributed Backend
      },
    },
    update: {},
    create: {
  ...
  console.log('   - Applications across all ATS stages (Applied, Under Review, Shortlisted, Interview, Selected)');
